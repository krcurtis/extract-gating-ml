...
convert_diva_gate :: DivaGate -> IntermediateGate
convert_diva_gate DivaGate{..} | (r_type dg_region) == RectangleRegion = undefined -- BasicRectangleGate{..}
{-
  where
    rg_id = dg_name
...
-}
convert_diva_gate DivaGate{..} | (r_type dg_region) == PolygonRegion = undefined -- BasicPolygonGate{..}
{-
  where
    pg_id = dg_name
